...
// XILE-PORT: browser I/O layer — this class runs in the renderer here, not Electron's main
// process. fetch() replaces fs reads; a build-time manifest.json (list of dataset filenames,
// see scripts/sync-xilehud-data.mjs) replaces directory listing; existsSync is answered by the
// manifest fetch itself. Everything below these shims is upstream code.
const fsp = {
    ...
    readdir: async (dir: string): Promise<string[]> => {
        ...
        if (!res.ok) throw new Error(`HTTP ${res.status} for ${dir}/manifest.json`);
        return res.json();
    },
};
const fs = { existsSync: (_p: string) => true }; // XILE-PORT: readdir/readFile surface real errors instead
const path = { join: (...parts: string[]) => parts.join('/') };
// XILE-PORT: import specifiers only — upstream's NodeNext ".js" suffixes dropped for Vite resolution.
import { getVersionConfig, getVirtualCategories, isBaseDomain, type OverlayGameVersion } from './config/modifierVersionConfig';
